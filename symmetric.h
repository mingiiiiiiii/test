...
// #define haetae_shake256_absorb_twice HAETAE_NAMESPACE(haetae_shake256_absorb_twice)
void haetae_shake256_absorb_twice(keccak_state* state, const uint8_t* in1, size_t in1len, const uint8_t* in2,
																	size_t in2len);

#define XOF256_BLOCKBYTES SHAKE256_RATE

#define xof256_absorbe_once(STATE, IN, IN_LEN) shake256_absorb_once(STATE, IN, IN_LEN)
#define xof256_absorbe_twice(STATE, IN, IN_LEN, IN2, IN2_LEN) \
	haetae_shake256_absorb_twice(STATE, IN, IN_LEN, IN2, IN2_LEN)
#define xof256_squeeze(OUT, OUT_LEN, STATE) shake256_squeeze(OUT, OUT_LEN, STATE)
#define xof256_squeezeblocks(OUT, OUTBLOCKS, STATE) shake256_squeezeblocks(OUT, OUTBLOCKS, STATE)
...
// #define haetae_shake128_stream_init HAETAE_NAMESPACE(haetae_shake128_stream_init)
void haetae_shake128_stream_init(keccak_state* state, const uint8_t seed[SEEDBYTES], uint16_t nonce);

// #define haetae_shake256_stream_init HAETAE_NAMESPACE(haetae_shake256_stream_init)
void haetae_shake256_stream_init(keccak_state* state, const uint8_t seed[CRHBYTES], uint16_t nonce);

#define STREAM128_BLOCKBYTES SHAKE128_RATE
#define STREAM256_BLOCKBYTES SHAKE256_RATE

#define stream128_init(STATE, SEED, NONCE) haetae_shake128_stream_init(STATE, SEED, NONCE)
#define stream128_squeezeblocks(OUT, OUTBLOCKS, STATE) shake128_squeezeblocks(OUT, OUTBLOCKS, STATE)
#define stream256_init(STATE, SEED, NONCE) haetae_shake256_stream_init(STATE, SEED, NONCE)
